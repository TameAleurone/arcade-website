...
(function(){
  let canvas,ctx,container,W=480,H=360,p1,p2,ball,keys={},score1,score2,animId,winner,mode,difficulty,aiReactionTimer,aiTargetY,serveTimer,record,online=null,mySide=null,lastNet=0,remoteKeys={up:false,down:false};
  ...
  function loop(){const dt=16.67;if(mode==='online'&&ArcadeOnline.isHost()){simulate(dt);if(performance.now()-lastNet>50){ArcadeOnline.send({type:'state',state:netState()});lastNet=performance.now();}}else if(mode!=='online'){simulate(dt);}draw();hud();animId=requestAnimationFrame(loop);}
  function startMatch(m,d){mode=m;difficulty=d||'Hard';document.getElementById('pong-setup').style.display='none';document.getElementById('pong-play').style.display='block';newMatch();setupTouchControls();if(!animId)animId=requestAnimationFrame(loop);}
